...
namespace StatisticMod
{
    public class Main : Mod
    {
        ...
        public void Start()
        {
            ...
            (harmony = new Harmony("com.aidanamite.StatisticMod")).PatchAll();
            if (canvas)
        ...
        public string ExtraSettingsAPI_HandleSliderText(string name, float value) => $"<color=#{GetColor(Mathf.RoundToInt(value)).GetHex()}>{(name.StartsWith("Raft") ? "◈" : "ALERT")}</color>";
    }
        ...
        void Update()
        {
            ...
            if (wm && sc && wm.GetCurrentWeather())
            {
                ...
                if (changes.GetValue(this, out _))
                {
                    ...
                    {
                        LineStart();
                        builder.Append("Foundation Count: ");
                    ...
                    {
                        LineStart();
                        builder.Append("Shark respawning in: [");
                        var flag = false;
                    ...
                    {
                        LineStart(2);
                        builder.Append("Next Story Location: ");
    ...
    public class TimeBar : MonoBehaviour
    {
        ...
        public static TimeBar Create(GameObject parent, float size, float X, float Y)
        {
            ...
            var lineWidth = size;
            var border = 3f;
            var barX = lineWidth / 2 + border + X;
            var barY = lineWidth / 2 + border - Y;
    ...
    public class SharkIndicator : MonoBehaviour
    {
        ...
        public static SharkIndicator Create(AI_StateMachine_Shark shark)
        {
            ...
            o.timePassed = 0;
            o.shark = shark;
            o.text = Main.CreateText(o.transform, rect.sizeDelta.x * rect.localScale.x * 0.45f, rect.sizeDelta.y * rect.localScale.y * 0.45f, "alert", Main.canvas.dropText.fontSize * 2, Color.red, rect.sizeDelta.x * rect.localScale.x * 0.06f, Main.canvas.dropText.fontSize * 3, Main.canvas.dropText.font).GetComponent<Text>();
